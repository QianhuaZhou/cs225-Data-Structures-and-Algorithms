...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());//middle item is split up and no longer exist
    new_right->children.assign(mid_child_itr, child->children.end());//middle item still exist
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->children[first_larger_idx]->elements.size() > order - 1) split_child(subroot, first_larger_idx);
